...
THE BAR IS THE OPPOSITE WAY ROUND FROM THE OLD CFF BUILD. The font ships as
                  TrueType (`glyf`), so Bai Jamjuree's Thai outlines are copied
                  verbatim and **Thai must be pixel-exact**. Latin now carries
                  the cubic->quadratic conversion and the engine change, bounded
                  by LATIN_MAX_MEAN_DEV.

                  Slussen's CFF stem hints (2-6 per Latin glyph) are dropped by
                  the format change and nothing replaces them. FreeType is run
                  unhinted here so this test cannot see that; it is a Windows
                  small-size question, not a Linux one.
...
# Latin tolerances, set from the measured distribution over the four weights:
# mean edge deviation p50 2.69, p99 9.05, max 11.51/255; zero advance mismatches.
# Thai has no tolerance at all — it must be byte-identical to Bai Jamjuree.
LATIN_MAX_MEAN_DEV = 20.0
LATIN_MAX_DIM_DELTA = 1
LATIN_MAX_ADV_DELTA = 0
...
def run():
    ...
    print("    Latin shapes as Slussen; raster within the measured conversion bound")
